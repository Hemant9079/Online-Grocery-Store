...
const Dairy_Products = () => {
  ...
  const staticProducts = allProducts.filter(p => p.category === 'Dairy');
  const { products: dynamic } = useDynamicProducts('Dairy');
  ...
  const allDairy = [...staticMapped, ...dynamicMapped];

  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '20px', padding: '20px' }}>
      {allDairy.map((product, index) => (
        <Lists
